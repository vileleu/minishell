...
char	*replace(char *line, int *i, char *s, t_o *o)
{
	char	*beg;
	char	*end;
	int		len;
...
char	*not_ev_bis(char *l, int *i, int v, t_o *o)
{
	char	*tmp;
...
char	*not_ev(char *l, int *i, int v, t_o *o)
{
	int		j;
	int		tmp;
...
int		ev_strdup(char **line, int *i, t_o *o, int v)
{
	...
	while (tmp)
	{
		...
		{
			if (!(*line = replace(*line, i, tmp->content, o)))
				return (0);
			return (1);
		}
		tmp = tmp->next;
	...
	{
		if (!(*line = replace(*line, i, o->ret, o)))
			return (0);
		return (1);
	}
	else
	{
		if (!(*line = not_ev(*line, i, v, o)))
			return (0);
	}
	return (1);
}
